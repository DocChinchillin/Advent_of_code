...
def findBasins(basin, imax, jmax, mat):
    pravi = set()
    looked = set()
    ...
    while(basin):
        ...
        if x-1 >= 0 and (x - 1,y) not in looked:
            basin.add((x - 1,y))

        if y-1 >= 0 and (x,y - 1) not in looked:
            basin.add((x,y - 1))

        if imax >= x+1 and (x + 1,y) not in looked:
            basin.add((x + 1,y))

        if jmax >= y+1 and (x,y + 1) not in looked:
            basin.add((x,y + 1))
...
def main():
    ...
    for i in range(sizey):
        for j in range(sizex):
            low.add(lowPoint(i,j,mat,sizey - 1,sizex - 1))
    ...
    for x,y in low:
        ...
        sizes.append(len(findBasins(bas,sizey - 1,sizex - 1,mat)))
